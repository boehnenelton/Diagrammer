...
def create_app(overrides=None):
    ...
    app.config["MFDB_ROOT"] = Path(app.config["MFDB_ROOT"])
    app.config["MFDB_DB_DIR"] = Path(app.config["MFDB_DB_DIR"])
...
class DiagramStore:
    ...
    def load(self):
        ...
        return {
            "diagram_name": manifest_doc.get("DB_Name", "Untitled Diagram"),
            "shapes": shapes,
...
def _write_entity(manifest_path, entity_name, records):
    ...
    entity_path = Path(current_app.config["MFDB_DB_DIR"]) / "data" / (
        "shapenode.bejson" if entity_name == "ShapeNode" else "connector.bejson"
    )
    bejson_core_atomic_write(str(entity_path), doc)


def _update_manifest_name(manifest_path, diagram_name):
    manifest_doc = json.loads(Path(manifest_path).read_text(encoding="utf-8"))
    manifest_doc["DB_Name"] = diagram_name or "Untitled Diagram"
...
def save(self, payload):
    ...
    _update_manifest_name(manifest_path, payload.get("diagram_name"))
